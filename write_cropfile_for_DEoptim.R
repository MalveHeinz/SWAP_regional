# This script is used to write a crop file with placeholders for use-defined parmaters that can be optimized within the DEoptim_paramter_optimization.R script 


#_______________________________________________________________________________
#### Optimization of biomass partitioning (FOTB, FLTB FSTB and FRTB) via implementation of logistic functions
#_______________________________________________________________________________

## If you want to optimized paramters of the logistic functions, you will ned this part. If not, you can outcommend it.
# Den et al. 2022 suggested to use these functions instead of look-up tables as they provide a more realistic representation of biomass allocation to the different plant organs:
# Den, T. t., van de Wiel, I., de Wit, A., van Evert, F. K., van Ittersum, M. K., & Reidsma, P. (2022). Modelling potential potato yields: Accounting for experimental differences in modern cultivars. European Journal of Agronomy, 137. https://doi.org/10.1016/j.eja.2022.126510 
 
logistic_sigmoid <- function(x, a=1.0, b=1.0, c=0.0, d=0.0){
  r = a * 1/(1 + exp(b*(-x + c))) + d
  return(r)
}

FOTB <- function(x,FOTBa=10,FOTBb=1.2,FOTBc=1.0){

  FO <- logistic_sigmoid(x, a=FOTBc,  b=FOTBa, c=FOTBb)
  if (FO >= 0.99) {
    FO = 1.0
  } else if (FO <= 0.01){
    FO = 0.0
...
FLTB = function(x,FLTBa=5,FLTBb=1.0,FLTBc=0.85,FLTBd=0.0){
  FL = logistic_sigmoid(x, a=(FLTBc-FLTBd),
                        b=-FLTBa, c=FLTBb, d=FLTBd)
  if (FL >= 0.99) {
    FL = 1.0
  } else if (FL < 0.01){
    FL = 0.0
...
fltb.correction<-  function(x,y){
  if (any((x + y) > 1.0)){
    x = min(x, 1.0-y)
    warning("Warning: adjusting for negative partioning values")
  }
  return(x)
}

 # other parameters of the partitioning function that remain fixed, just out-comment them
FLTBa<-20
#FLTBb<-1.05 #is being calibrated
#FLTBc<-0.83
FOTBa<-7
FOTBb<-1.05
FLTBd<-0
...
#  compute partitioining functions, adjust for the parameters i am currently optimizing
# here you define the development stages you want to calculate the parameters for (0, 1, 1.27 etc)
part = data.frame(DVS=c(0.00,1.00,1.27,1.36,2.00))
#part = data.frame(DVS=seq(0,2,0.1))
part$FRTB_v <- c(FRTB,FRTB,FRTB,0,0)
part$FOTB_v <- mapply(FOTB,part$DVS, as.numeric(FOTBa),as.numeric(FOTBb),as.numeric(FOTBc))
part$FLTB_v <- mapply(FLTB,part$DVS, as.numeric(FLTBa),as.numeric(FLTBb),as.numeric(FLTBc))
part$FLTB_v <- mapply(fltb.correction, part$FLTB_v, part$FOTB_v)
part$FSTB_v <- 1.0 - part$FOTB_v - part$FLTB_v
#write.csv(part, file="C:/malve/DEopt_KOBOsoil/plots/part.csv")

part$check  <- format(FRTB[1]+(part$FLTB_v+part$FSTB_v+part$FOTB_v)*(1.0-part$FRTB_v)-1.0,nsmall = 10)

#_______________________________________________________________________________
### calibration of TSUMS
#_______________________________________________________________________________

#### I decided to also calibrate the TSUMS, since they govern at which time which DVS is reached (and therefore the course of many other parameters
# I defined a total TSUM (1942, from observed data TSUMS of average growing seasons in my study area)
# Then I only calibrate TSUMEA and calculate TSUMAM as follows:
TSUMAM<-(1942-TSUMEA)

#_______________________________________________________________________________
### writing the actual Textfile 
#_______________________________________________________________________________

# SWAP is sensitive to digits, you always need to input numbers as floating (=1.0), not integers (=1), which is why I use the format() function

# path where to store the  crop txt file that is overwritten each time 
path.crp<-paste0(homedir,"crop_files/PotatoD.crp" )
...
tx=c("***********************************************************************************************",
    ...
    #paste0("0.00 ",format(as.numeric(SLATB),nsmall=1),""),
    paste0("0.0      40.0     ",format(as.numeric(CF),nsmall=1),""),
    paste0("1.0      40.0     ",format(as.numeric(CF)*1.1,nsmall=1),""),
    paste0("2.0      50.0     ",format(as.numeric(CF)*1.1,nsmall=1),""),
    # "1.0      40.0    1.0",
    # "1.0      40.0    1.1",
    # "2.0      50.0    1.1",
    "* End of table",
    ...
    paste0("TSUMEA = ",format(as.numeric(TSUMEA),nsmall=1)," ! Temperature sum from emergence to anthesis [0..1d4 ºC, R]"),
    paste0("TSUMAM = ",format(as.numeric(TSUMAM),nsmall=1)," ! Temperature sum from anthesis to maturity  [0..1d4 ºC, R]"),
    "",
    ...
    #paste0("RGRLAI = ",format(as.numeric(RGRLAI),nsmall=4)," ! Maximum relative increase in LAI [0..1 m2/m2/d, R]"),
    "***********************************************************************************************",
    "",
    "***********************************************************************************************",
    ...
    #paste0("SPAN   =  ",format(as.numeric(SPAN),nsmall=1)," ! Life span under leaves under optimum conditions [0..366 d, R]"),
    ...
    # "2.00 0.0015",
    paste0("0.00 ",format(as.numeric(SLATB),nsmall=1),""),
    paste0("1.10 ",format(as.numeric(SLATB),nsmall=1),""),
    paste0("2.00 ",format(as.numeric(SLATB)*0.5,nsmall=1),""),
    "* End of Table ",
    "***********************************************************************************************",
    "",
    "***********************************************************************************************",
    ...
    # "1.57  30.0",
    # "2.00   0.00",
    paste0("0.00  ",format(AMAXTB, nsmall=1),""),
    paste0("1.57  ",format(AMAXTB, nsmall=1),""),
    "2.00   0.00",
    "* End of table ",
    "",
    ...
    "",
    "Q10    =  2.0 ! Increase in respiration rate with temperature  [0..5 -/10 C, R]",
    ...
    "FRTB = ",
    paste0("0.00   ",format(as.numeric(FRTB),nsmall=1),""),
    paste0("1.00   ",format(as.numeric(FRTB),nsmall=1),""),
    # "0.00   0.20",
    # "1.00   0.20",
    "1.36   0.00",
    "2.00   0.00",
    ...
    # "1.4    0.00",
    # "2.00   0.00",
    paste0(sprintf("%.2f",part$DVS[1]),"  ",sprintf("%.6f",part$FLTB_v[1]),""),
    paste0(sprintf("%.2f",part$DVS[2]),"  ",sprintf("%.6f",part$FLTB_v[2]),""),
    paste0(sprintf("%.2f",part$DVS[3]),"  ",sprintf("%.6f",part$FLTB_v[3]),""),
    paste0(sprintf("%.2f",part$DVS[4]),"  ",sprintf("%.6f",part$FLTB_v[4]),""),
    paste0(sprintf("%.2f",part$DVS[5]),"  ",sprintf("%.6f",part$FLTB_v[5]),""),
    "* End of table ",
    "",
    ...
    # "1.4    0.102477",
    # "2.00   0.00",
    paste0(sprintf("%.2f",part$DVS[1]),"  ",sprintf("%.6f",part$FSTB_v[1]),""),
    paste0(sprintf("%.2f",part$DVS[2]),"  ",sprintf("%.6f",part$FSTB_v[2]),""),
    paste0(sprintf("%.2f",part$DVS[3]),"  ",sprintf("%.6f",part$FSTB_v[3]),""),
    paste0(sprintf("%.2f",part$DVS[4]),"  ",sprintf("%.6f",part$FSTB_v[4]),""),
    paste0(sprintf("%.2f",part$DVS[5]),"  ",sprintf("%.6f",part$FSTB_v[5]),""),
    "* End of table ",
    "",
    ...
    # "1.4    0.897523",
    # "2.00   1.00",
    paste0(sprintf("%.2f",part$DVS[1]),"  ",sprintf("%.6f",part$FOTB_v[1]),""),
    paste0(sprintf("%.2f",part$DVS[2]),"  ",sprintf("%.6f",part$FOTB_v[2]),""),
    paste0(sprintf("%.2f",part$DVS[3]),"  ",sprintf("%.6f",part$FOTB_v[3]),""),
    paste0(sprintf("%.2f",part$DVS[4]),"  ",sprintf("%.6f",part$FOTB_v[4]),""),
    paste0(sprintf("%.2f",part$DVS[5]),"  ",sprintf("%.6f",part$FOTB_v[5]),""),
    "* End of table",
    "***********************************************************************************************",
    "",
    "***********************************************************************************************",
    ...
    # "Q10_microbial       = ",format(as.numeric(Q10_microbio),nsmall=1),"          ! Relative increase in microbial respiration at temperature rise of 10 ºC [1.0..4.0 -, R]",
    "Q10_microbial       = 2.8          ! Relative increase in microbial respiration at temperature rise of 10 ºC [1.0..4.0 -, R]",
    "Specific_resp_humus = 1.6d-3         ! Respiration rate of humus at 25 ºC [0.0..1.0 kg O2/kg ºC/d, R] ",
    # "SRL                 = ",format(as.numeric(SRL),nsmall=1),"      ! Specific root length [0.d0..1d10 (m root)/(kg root), R]    ",  
    "SRL                 = 151375.0      ! Specific root length [0.d0..1d10 (m root)/(kg root), R]    ",  
    ...
    "HLIM4  =   -10000.0 ! No water extraction at lower soil water pressure heads [-2d4..100 cm, R]", # ",format(HLIM4, nsmall=1),"
    # paste0("HLIM4  =   ",format(as.numeric(HLIM4),nsmall=1)," ! No water extraction at lower soil water pressure heads [-2d4..100 cm, R]"), # ",format(HLIM4, nsmall=1),"
    # "ADCRH  =   ",format(as.numeric(ADCRH),nsmall=1),"    ! Level of high atmospheric demand, corresponding to HLIM3H [0..5 cm/d, R] ",
    "ADCRH  =       0.5    ! Level of high atmospheric demand, corresponding to HLIM3H [0..5 cm/d, R] ",
    "ADCRL  =   ",format(as.numeric(ADCRL),nsmall=1),"    ! Level of high atmospheric demand, corresponding to HLIM3H [0..5 cm/d, R] ",
    #"ADCRL  =       0.1    ! Level of low atmospheric demand, corresponding to HLIM3L [0..5 cm/d, R] ",    
    #"ALPHACRIT =    1.0  ! Critical stress index (Jarvis, 1989) for compensation of root water uptake [0.2..1 -, R]  ", #  ",format(ALPHACRIT, nsmall=1),"
    paste0("ALPHACRIT =   ",format(ALPHACRIT, nsmall=1),"  ! Critical stress index (Jarvis, 1989) for compensation of root water uptake [0.2..1 -, R]  "), #  ",format(ALPHACRIT, nsmall=1),"
    "",
    ...
    "RDC    =   ",format(as.numeric(RDC),nsmall=1)," ! Maximum rooting depth of particular cultivar [0..1000 cm, R]",
    #"RDC    =    50.0 ! Maximum rooting depth of particular cultivar [0..1000 cm, R]",
    "",
    ...
    "RDCTB =             ",  
    # "0.0,",(RDCTB*log(1)+0.9966),"",
    # "0.1,",(RDCTB*log(2)+0.9966),"",
    # "0.2,",(RDCTB*log(3)+0.9966),"",
    # "0.3,",(RDCTB*log(4)+0.9966),"",
    # "0.4,",(RDCTB*log(5)+0.9966),"",
    # "0.5,",(RDCTB*log(6)+0.9966),"",
    # "0.6,",(RDCTB*log(7)+0.9966),"",
    # "0.7,",(RDCTB*log(8)+0.9966),"",
    # "0.8,",(RDCTB*log(9)+0.9966),"",
    # "0.9,",(RDCTB*log(10)+0.9966),"",
    # "1.0,",(RDCTB*log(11)+0.9966),"",
    "0.0  1.0",
    "0.1  0.741",
    "0.2  0.549",
    "0.3  0.407",
    ...
    "* Part 15: General",
    "",
    "SCHEDULE = 1  ! Switch for application irrigation scheduling [Y=1, N=0] ",
    "",
    "* If SCHEDULE = 0, no more information is required in this input file!", 
    "* If SCHEDULE = 1, continue ....",
    "",
    "STARTIRR = 10 6 ! Specify day and month at which irrigation scheduling starts [dd mm]",
    "ENDIRR  = 10 8 ! Specify day and month at which irrigation scheduling stops [dd mm]",
    ...
    # paste0("0.0  ",format(TAW, nsmall=1)), 
    # paste0("2.0  ",format(TAW, nsmall=1)), 
    "0.0  0.45",
    "2.0  0.45",
    "* End of table",
    "",
    ...
    "irgdepmax = 35.0    !  Maximum irrigation depth [irgdepmin..1d7 mm, I]",
    "",
    "",
    "* End of .crp file ",
    "")

writeLines(tx, con=path.crp)
#_______________________________________________________________________________
# End, now you have overwritten the old cropfile in the specified directory
#_______________________________________________________________________________
